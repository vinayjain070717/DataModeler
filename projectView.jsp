...
this.maxWidth="";
this.defaultSize="";
this.maxWidthOfPrecision="";
this.allowAutoIncrement="";
}
function SaveProject()
{
var project=new Project();
console.log(dsProject.project.code);
project.code=dsProject.project.code;
project.title=dsProject.project.title;
project.dateOfCreation=dsProject.project.dateOfCreation;
project.timeOfCreation=dsProject.project.timeOfCreation;
project.databaseArchitecture=dsProject.project.databaseArchitecture;
var dTables=[];
for(var i=0;i<tableComponents.length;i++)
{
console.log("table");
var dTable=new dsTable();
var table=tableComponents[i].databaseTable;
var tableDrawable=tableComponents[i].tableDrawable;
dTable.code=table.code;
dTable.name=table.name;
var engine=new Engine();
//changes
// if(table.engine!=undefined) engine.code=table.engine;
// else engine.code=$("#engine").val();
engine.code=10;
dTable.engine=engine;
//changes
if(table.note!=undefined) dTable.note=table.note;
else dTable.note="";
dTable.xlocation=tableDrawable.x;
dTable.ylocation=tableDrawable.y;
var dFields=[];
for(var j=0;j<table.fields.length;j++)
{
console.log("field");
var dField=new dsField();
console.log(table.fields[j]);
var field=table.fields[j];
dField.name=field.name;
var dataType=new DataType();
dataType.dataType=field.dataType;
dField.dataType=dataType;
dField.width=field.dataTypeSize;//data type Size //improvement needed
// console.log("data type size "+field);
dField.numberOfDecimalPlaces=0;
if(field.constraint.search("PK")!=-1) dField.isPrimaryKey=true;
if(field.constraint.search("AI")!=-1) dField.isAutoIncrement=true;
if(field.constraint.search("UN")!=-1) dField.isUnique=true;
if(field.constraint.search("NN")!=-1) dField.isNotNull=true;
dField.defaultValue=field.defaultValue;
dField.checkConstraint="";
if(field.note!=undefined) dField.note=field.note;
else field.note="";
dFields.push(dField);
}
dTable.field=dFields;
dTables.push(dTable);
}
project.table=dTables;
project.canvasWidth=canvas.width;
project.canvasHeight=canvas.height;
var obj={"argument-1":project};
console.log(obj);
$.ajax("/tmdmmodel/webservice/projectService/saveProject",{
type: "POST",
...
{
console.log("Save return : "+res.result);
}
else console.log("work done");
}
else
{
if(res.isException) alert("Exception : "+JSON.stringify(res.exception));
...
function fetchProjectDS()
{
canvas.width=dsProject.project.canvasWidth;
canvas.height=dsProject.project.canvasHeight;
var databaseTable;
var tableDrawable;
var field;
for(var i=0;i<dsProject.project.table.length;i++)
{
var vTable=dsProject.project.table[i];
databaseTable=new DatabaseTable(vTable.name);
databaseTable.code=vTable.code;
databaseTable.engine=vTable.engine.code;
databaseTable.note=vTable.note;
for(var j=0;j<vTable.field.length;j++)
{
var vField=vTable.field[j];
field=new Field();
field.serialNumber=j+1;
field.name=vField.name;
field.dataType=vField.dataType.dataType;
field.dataTypeSize=vField.width;
var constraintString="";
if(vField.isPrimaryKey) constraintString+="PK ";
if(vField.isAutoIncrement) constraintString+="AI ";
if(vField.isUnique) constraintString+="UN ";
if(vField.isNotNull) constraintString+="NN ";
//field.constraint=vField.checkConstraint;
...
tableDrawable=new TableDrawable(vTable.xlocation,vTable.ylocation,databaseTable);
...
for(var i=0;i<tableComponents.length;i++)
{
console.log(tableComponents[i]);
//tableComponents[i].draw();
}
draw();
}
function loadProjectDS()
{
window.dsProject=null;
$.ajax("/tmdmmodel/webservice/projectService/loadProjectDS",{
type: "GET",
contentType: "application/json",
success: function(res)
{
...
console.log(a);
dsProject=res.result;
//console.log(dsProject.project.table[0].name);
fetchProjectDS();
}
else console.log("work done");
}
...
{
if(res.isException) console.log("Exception : "+res.exception);
else console.log("error");
}
},
error: function(err){
...
{
//if($("#tableName").val('')) $("#tableName").val("Table ".concat(defaultTableName));
...
if(tableComponents.length>=1) window.defaultTableName=tableComponents.length;
...
{
$("#attributeName").removeClass("is-invalid");
$("#dataType").removeClass("is-invalid");
$("#dataTypeSize").removeClass("is-invalid");
}
function validateAddForm()
{
var error=false;
if($("#attributeName").val()=="" || $("#attributeName").val().search(" ")!=-1)
{
error=true;
$("#attributeName").addClass("is-invalid");
}
if($("#dataType").val()=="-1")
{
error=true;
...
{
error=true;
$("#dataTypeSize").addClass("is-invalid");
}
if($("#defaultValue").val()=="")
{
error=true;
$("#defaultValue").addClass("is-invalid");
}
if($("#attributeNote").val()=="")
{
$("#attributeNote").val(" ");
}
return error;
...
function AddEngineFunction()
{
console.log("add engine function invoked");
//console.log(databaseTable.engine);
console.log($("#engine").attr("name"));
databaseTable.engine=$("#engine").val();
}
function AddTableNoteFunction()
{
console.log("add table note function invoked");
databaseTable.note=$("#tableNote").val();
}
function GenerateScriptFunction()
{
	var a={}
	var projectTitle=document.getElementById("projectTitleHidden").value;
	a["argument-1"]=projectTitle;
	$.ajax("/tmdmmodel/webservice/dmodelUtility/generateScript",{
		type:"post",
		contentType:"application/json",
		data: JSON.stringify(a),
		success: function(res){
			console.log(res.result);
			$("#generateScriptModal .modal-body").text(res.result);
...
<input type="hidden" id="projectTitleHidden" value="${currentProject.code}" />
...
<div class="btn-group mr-2" role="group" aria-label="First group">
<button class="btn btn-lg" onclick='GenerateScriptFunction()'>
...
<button type="button" class='btn btn-success'  data-dismiss="modal" onclick='SaveProject()'>Yes</button>
...
<div class="modal fade" id="generateScriptModal" role="dialog">
	<div class="modal-dialog">
	<div class="modal-content">
	<div class="modal-header">
	<h4 class="modal-title">Sql Script</h4>
	<button type="button" class="close" data-dismiss="modal">&times;</button>
	</div>
	<div class="modal-body">
	<p>
	...
	<div class="modal-footer">
	<button type="button" class="btn btn-default" >Copy to Clipboard</button>	
...
<h6>Engine&nbsp;&nbsp;
<select name='engine' id="engine" onchange='AddEngineFunction()'>
...
<option value='${en.code}' class='btn btn-light btn-block'>${en.name}</option>
</c:forEach>
</c:if>
</c:forEach>
</select>
...
<textarea class="note" id="tableNote" rows="2" cols='200' placeholder='Note' style='width:300px;' oninput='AddTableNoteFunction()'></textarea>
...
<div style='overflow-y:scroll;height:150px;display:block;border:1px solid black;'>
...
<div class='col-md-5'>
<div class="form-group">
<textarea class="note" id="attributeNote" name="attributeNote" rows="2" cols='40' placeholder='Note' ></textarea>
<label for="attributeNote"></label>
</div>
</div>
<div class='col-md-1'>
</div>
<button type='button' class='btn btn-default col-md-2' onclick='addAttributeData()' style='background-color:#000000;'>
<i class='fas fa-plus' style='font-size:3em;color:#ffffff;'></i>
</button>
<div class='col-md-1'>
</div>
</div><!-- Form Row Ends-->
